...
def get_np_image_from_path(image_path, height=299, width=299, channels=3):
    return process_image(np.asarray(cv2.imread(image_path)), height=height, width=width, channels=channels)
...
def get_np_image_from_url(image_url, timeout=5, height=299, width=299, channels=3):
    ...
    return process_image(cv2.imdecode(np_arr, cv2.IMREAD_COLOR), height=height, width=width, channels=channels)
...
def process_image(image_tensor=None, height=299, width=299, channels=3, normalize=True):
    """
    crop, resize and normalize the input image Tensor
    :param image_tensor:
    :param height:
    :param width:
    :param channels:
    :param normalize: 是否归一化处理
    :return: shape为[1, height, width, channels]的numpy张量
    """
...
def load_image_tf(filename, label, height, width, channels=3, normalize=True):
    ...
    :param channels:
    :param normalize:
    :return: (height, width, channels) shape 的张量
    """
