...
if __name__ == '__main__':
    ...
    mnist_classifier = tf.estimator.Estimator(model_fn=cnn_model_fn, model_dir="./mnist_transfer_model", params={
        ...
        "nb_classes": 2,
        "checkpoints": [
            (os.path.join(MODELS_DIR, "models", "mnist", "mnist_model"), True),  # (dir, load_dense_layer)
        ]
    })
    ...
    mnist_classifier = tf.estimator.Estimator(model_fn=cnn_model_fn, model_dir="./mnist_finetune_model", params={
        "finetune": True,
        "nb_classes": 2,
        "checkpoints": [(os.path.join(MODELS_DIR, "models", "mnist", "mnist_model"), True),  # (dir, load_dense_layer)
                        (os.path.join(MODELS_DIR, "models", "mnist", "mnist_transfer_model"), False)
                        ]
    })
